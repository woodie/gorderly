...
// `describe := context` if a file actually calls describe(...) somewhere,
// e.g. to name the method/feature under test; most files here only ever
// use context(...) and don't need the alias at all.
// it's hook methods are called qualified: it.Before/it.After.
